...
import { Circle, CheckCircle2, XCircle, Cpu, PenSquare, Menu, CircleEllipsis, StopCircle, StopCircleIcon } from "lucide-react"
      ...
      <ScrollArea className={`h-[calc(100vh-64px)] ${BREAKPOINT}:h-[calc(100vh-80px)]`}>
        <div className="px-4 py-2">
          ...
          {tasks.map((task) => {
            ...
            const { icon: StatusIcon, color } = statusIcons[task.status] || { icon: CircleEllipsis, color: 'text-blue-500' };
            return (
              <Link
                key={task.id}
                href={`/tasks/${task.id}`}
